...
shortened so the line fits your terminal: the width budget is read from the `COLUMNS`
environment variable that Claude Code exports to the script (v2.1.153+), with a fixed
100-column fallback for older builds and manual runs.
...
| Segment | Meaning |
|---------|---------|
| `Op4.7 (1m)` | Current model, abbreviated: family (`Op`/`So`/`Ha`/`My`/`Fb`) glued to the version + context size (dim). `Opus 4.7 (1M context)` becomes `Op4.7 (1m)`, `Fable 5` becomes `Fb5`. The family is colored by cost tier — `Ha` green, `So` yellow, `Op` orange, `Fb`/`My` bold red — so the left edge of the line answers "how expensive is this session" at a glance. Unrecognised model names are shown as-is (dim) |
| `Op4.7:hg` | Reasoning-effort tier as a lowercase code colon-glued to the model: `lo`/`md`/`hg`/`xhg`/`mx` for `low`/`medium`/`high`/`xhigh`/`max`, colored by the same cost scale (`lo` green → `md` yellow → `hg` orange → `xhg` red → `mx` bold red; unknown levels dim). Reflects the live value (including mid-session `/effort` changes); the context size trails it — `Op4.7:hg (1m)`. Shown only on models that report `effort.level`; absent on models that don't support reasoning effort |
| `style:expl` | Active output style, shown only when it differs from `default` (`Explanatory` → `expl`, `Learning` → `learn`, custom styles abbreviate to their first five chars). A non-default style changes how Claude responds, is stored per scope, and is easy to switch on and forget — this keeps it visible. Dim label, magenta value |
| `claude-…tusline (main)` | Working directory basename (dim) + current branch in cyan; shows `(HEAD@<sha>)` in red for detached HEAD. The basename is trimmed to a 15-char `head…tail` middle ellipsis (7 chars each side) **only when the full status line would otherwise exceed the width budget** (the terminal width from `COLUMNS`, 100 columns when it's absent) — short lines keep the full name |
...
А если хотите получше освоить Claude Code и вообще принцип работы AI, то у меня есть различные статьи на эту тему 
[https://ilia-pro-ai.com/](https://ilia-pro-ai.com/articles/)
