...
| `Op4.7:hg` | Reasoning-effort tier as a lowercase code colon-glued to the model (same dim shade): `lo`/`md`/`hg`/`xhg`/`max` for `low`/`medium`/`high`/`xhigh`/`max`. The one exception is `max`, rendered in red: max is the most expensive tier, rarely a default, and easy to leave switched on after a one-off `/effort max`. Reflects the live value (including mid-session `/effort` changes); the context size trails it — `Op4.7:hg (1m)`. Shown only on models that report `effort.level`; absent on models that don't support reasoning effort |
| `stl:expl` | Active output style, shown only when it isn't `default`: `expl`/`learn`/`conc`/`proact` for the built-in `Explanatory`/`Learning`/`Concise`/`Proactive`, custom styles lowercased and cut to 7 chars. A non-default style changes how Claude responds and is saved per project by `/config`, so it quietly differs between projects and is easy to forget. Dim label; the code takes the same pink as a low context bar |
